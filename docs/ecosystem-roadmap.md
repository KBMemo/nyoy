...
| webhook 通知 | リアルタイム re-embed | Nyoy 受信側・徒然 delivery 実装済み。development create/update/delete E2E 確認済み（[Memo RAG Webhook](./architecture/memo-rag-webhook.md)） |
...
| **3b′** | 徒然 PGroonga 検索（site DB） | **完了**（site DB 本番稼働） |
...
- `AgentGraph::Core` はRails非依存のprivate path gemへ分離済み。workflow registryと全標準roleの差し替え境界を検証し、別repository化は第二利用者または独立release要件が発生するまで保留する（[分離再評価](./architecture/agent-graph-core-extraction-assessment.md)）。
...
- [AgentGraph Intent Profile 実運用 Runbook](./agent-graph-intent-profile-runbook.md)

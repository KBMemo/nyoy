...
`gpt_oss` 接続 `http://balvenie:10011` の `/health` は成功した。一方、`/props` の `model_path` は `gemma-4-E4B-it-qat` を示しており、gpt-oss の比較条件を満たさなかった。このため実測値は未記録。gpt-oss をロードした接続先へ戻した後、第2節から再開する。

...
6. retry / resume API を検討する
   ...
   - **最初の実装は Research Graph の複製 retry に限定する。** `AgentGraph::RunRetryPlanner` による dry-run を failed run 詳細で表示し、「retry 可能性」「起点 checkpoint」「次 node」「ブロック理由」を確認できるようにする。`AgentGraph::RunRetryLauncher` は複製 run を作成し、UI の retry ボタンは `retryable` な dry-run の場合だけ表示する。write 系 Graph は引き続き保留表示に留める。

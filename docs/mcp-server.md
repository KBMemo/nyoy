...
| `list_image_generation_options` / `list_prompt_styles` | `sd_cpp` 有効（`list_prompt_styles` は互換用・非推奨。新規クライアントは `list_image_generation_options` を使用） |
...
- **画像生成フロー**: `list_image_generation_options` で style / model / template の ID を確認。`list_prompt_styles` は style 一覧のみを返す互換用の非推奨ツールであり、新規連携では使わない。ラフ案は `generate_image` → `get_image_generation`（`awaiting_selection`）→ `refine_image`（`draft_index`）→ `get_image_generation`（`completed`）。直接 txt2img は `generate_image(generation_flow=direct, sd_model_profile_id=...)` → `get_image_generation`（`completed`）。
